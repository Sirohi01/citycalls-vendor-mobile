...
class DashboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final pendingAcceptance =
        jobs.value?.where((j) => j.status == 'ASSIGNED_TO_EMPLOYEE').toList() ??
            [];
    final urgentCount = jobs.value
        ?.where((j) => j.priority == 'URGENT' || j.priority == 'HIGH')
        .length;
          ...
          children: [
            ...
            Consumer(
              builder: (context, ref, _) {
                ...
                final queued = pending.value
                        ?.where((a) => a.status != 'SYNCED')
                        .toList() ??
                    [];
                if (queued.isEmpty) return const SizedBox.shrink();
                final rejected =
                    queued.where((a) => a.status == 'REJECTED').length;
                return Padding(
                  ...
                  child: GlassCard(
                    ...
                    child: InkWell(
                      ...
                      child: Row(
                        children: [
                          Icon(rejected > 0 ? Icons.sync_problem : Icons.sync,
                              size: 18,
                              color: rejected > 0
                                  ? AppColors.urgent
                                  : AppColors.warning),
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text('New Job Requests (${pendingAcceptance.length})',
                            style: const TextStyle(
                                fontSize: 15.5, fontWeight: FontWeight.bold)),
                      ],
                    ),
                    const SizedBox(height: 4),
                    const Text('These need your Accept/Reject response.',
                        style:
                            TextStyle(fontSize: 12, color: AppColors.slate500)),
                    const SizedBox(height: 12),
                    Column(
                        children: pendingAcceptance
                            .map(
                                (j) => _JobPreviewCard(job: j, highlight: true))
                            .toList()),
                  ],
                ),
              ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  jobs.when(
                    data: (items) {
                      final rest = items
                          .where((j) => j.status != 'ASSIGNED_TO_EMPLOYEE')
                          .toList();
                      if (rest.isEmpty) {
                      ...
                      final todays = rest
                          .where((j) =>
                              ...
                              _isSameDay(j.scheduledDate!, today))
                          .toList()
                        ..sort((a, b) =>
                            a.scheduledDate!.compareTo(b.scheduledDate!));
                      final preview =
                          (todays.isNotEmpty ? todays : rest).take(4).toList();
                      return Column(
                          children: preview
                              .map((j) => _JobPreviewCard(job: j))
                              .toList());
                    },
                    loading: () => const Padding(
                        padding: EdgeInsets.symmetric(vertical: 30),
                        child: Center(child: CircularProgressIndicator())),
                    error: (err, __) => Text('Could not load jobs: $err',
            ...
            child: Stack(
              ...
              children: [
                const Positioned(
                    right: -60,
                    top: -60,
                    child: GlowBlob(color: AppColors.teal400, size: 200)),
                const Positioned(
                    left: -70,
                    bottom: -50,
                    child: GlowBlob(color: AppColors.cyan400, size: 170)),
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    profile.when(
...
class _BranchStrip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final parts = [profile.branchName, profile.subBranchName, profile.teamName]
        .where((s) => s != null)
        .cast<String>()
        .toList();
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(job.serviceName ?? 'Service Request',
                            style: const TextStyle(
                                fontWeight: FontWeight.w700, fontSize: 13.5),
                            overflow: TextOverflow.ellipsis),
                        const SizedBox(height: 2),
                        Text(
                          job.scheduledSlot != null
                              ? '${job.customerName ?? ''} • ${job.scheduledSlot}'
                              : job.customerName ?? '',
                          style: const TextStyle(
